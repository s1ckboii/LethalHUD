...
internal static class LethalConfigProxy
{
    ...
    [MethodImpl(MethodImplOptions.NoInlining | MethodImplOptions.NoOptimization)]
    public static void AddConfig(ConfigEntry<string> configEntry, bool requiresRestart = false, bool isHexColor = false)
    {
        BaseConfigItem baseConfigItem;

        if (isHexColor)
        {
            baseConfigItem = new HexColorInputFieldConfigItem(configEntry, requiresRestart);
        }
        else
        {
            baseConfigItem = new TextInputFieldConfigItem(configEntry, requiresRestart);
        }
    ...
    public static void AddConfig<T>(ConfigEntry<T> configEntry, bool requiresRestart = false)
    {
        ...
        if (configEntry.Description.AcceptableValues is AcceptableValueBase acceptableValue)
        {
            ...
            {
                AddConfigSlider(configEntry);
                return;
            }
            // Check if it is an AcceptableValueList for string
            else if (acceptableValue is AcceptableValueList<string>)
            {
                AddConfigDropdown(configEntry);
                return;
            }
        }
        // Handle enum config entries
        if (typeof(T).IsEnum)
        {
            Type enumDropDownType = typeof(EnumDropDownConfigItem<>).MakeGenericType(typeof(T));
            object enumDropDown = Activator.CreateInstance(enumDropDownType, configEntry, requiresRestart);
            MethodInfo addConfigItemMethod = AccessTools.Method(typeof(LethalConfigManager), nameof(LethalConfigManager.AddConfigItem), [enumDropDownType]);
            addConfigItemMethod.Invoke(null, [enumDropDown]);
            return;
        }
